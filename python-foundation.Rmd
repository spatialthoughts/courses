...
Once conda has been installed and configured, we will create a new *environment* and install the required packages. An environment is an isolated space where you will install required packages. Many packages may contain conflicting requirements, and it is a good practice to create a new environment for each of your projects and install only the required packages there. We will now type commands in a terminal to create a new environment. 

1, *(Windows users)* Search for Anaconda Powershell Prompt in the Start Menu and launch it. *(Mac/Linux users)*: Open a Terminal window.  Enter the command below and press *Enter* to create your new environment.

```
conda create --name python_foundation -y
```

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment1.png')
```

2. Now that the environment is created, you need to activate it. Type the command below and press *Enter*. Once the environment activates, the ``(base)`` will change to ``(python_foundation)``. 
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment2.png')
```

3. Now we are ready to install the required packages using the `conda install` command. We can now install other required packages for this class. Run the command below to install `geopandas`, `geopy`, `rioxarray`, `matplotlib` and `jupyterlab` packages. We use the [conda-forge](https://conda-forge.org/docs/user/introduction.html) channel which has a broader range of packages available than the default channel.
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment3.png')
```

4. Once the command finishes, you should see a screen such as below.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment4.png')
```

5. Your Python environment is now ready. Launch the *JupyterLab* application using the command below. This will initiate and run a local server in your system and opens in your default browser. 
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment5.png')
```
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment5a.png')
```
...
6. A new browser tab will open with an instance of JupyterLab. If prompted to *Select Kernel*, choose `Python 3 (ipykernel)` option. Check the *Always start the preferred kernel* box and click *Select*.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment6.png')
```

7. Enter the following statements in the first cell and click the *Run* button. **If nothing happens - it means your installation was successful!**. Your environment is now ready for the course. If you get an *ImportError*, repeat the installation steps carefully again.
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/environment7.png')
```
...
Sometimes if you have older version of packages in your base environment, it can cause conflicts when you try installing other packages. You can try updating your base environment and try again. The following sequence of commands will delete the python_foundation environment and update packages in your base environment.
...
Follow the installation steps again to create the environment and required packages.
...
LLM-based coding assistants are useful in writing and debugging Python code. There are many models and agents available and you may use your favorite coding assistant in this course. We recommend using Claude for this course. The exercises and assignments are tested using the free version (using Sonnet 4.6 model). You can sign-up for a free account at [Claude AI](https://claude.ai/).
...
Open the notebook named ``15_working_with_rioxarray.ipynb``.
...
```{r child='python-foundation-output/15_working_with_rioxarray.md'}
```
...
Once you get some experience with Python, we suggest graduating to a professional editor. We recommend [Visual Studio Code (VS Code)](https://code.visualstudio.com/) - which can work with both text files and Jupyter Notebooks and has in-editor support for AI-Assisted coding.
...
* [Data-driven Framework for Quantifying Helicopter Demand](https://colab.research.google.com/github/SiloSolutionist/Abitibi-QC-Heli-Risk-Analysis/blob/main/Abitibi_QC_Heli_Logistics_Risk_Analysis.ipynb) by Kayla Helt
* [Karnataka Assembly Election Dashboard](https://karnatakaassemblyresultsdashboard.netlify.app/) by Srinidhi HV
* [Geocoding Addresses from a spreadsheet](https://colab.research.google.com/github/KlausAgerskov/Python_geocode_internships/blob/master/klaus_danish_geocode_final.ipynb) by Klaus Agerskov
* [Retrieving and visualizing data from OpenStreetMap](https://colab.research.google.com/github/luicarboni/My_Projects/blob/master/Starting_with_OSMnx.ipynb) by Luigi Carboni
* [Route Optimization](https://colab.research.google.com/github/amitkb3/python_spatial_analysis_final_project/blob/master/PythonSpatialAnalysisCourse_FinalProject.ipynb) by Amit Bothra
...
* [Analyzing Music using Spotify API](https://colab.research.google.com/github/deepikajpsharma/Spotify/blob/main/Spotify%20data%20analysis.ipynb) by Deepika Sharma
* [Analyzing Primate Distribution in Protected Areas](https://github.com/ulfboge/galago-wdpa/tree/main) by Johan Karlsson
* [Finding Solar Sites from OpenStreetMap](https://colab.research.google.com/github/cosmith57/Python-Foundations-for-Spatial-Analysis/blob/main/Colby_Smith_PFSA.ipynb) by Colby Smith
* [Saline System Visualizer](https://colab.research.google.com/github/valegio/saline_map/blob/main/saline_map.ipynb) by Valentina Giovanetti
...
- **Geo-processing**: I have several intermediate level geoprocessing tutorials that use QGIS. You can take any of the following tutorials and implement it using libraries such as *geopandas* and *rioxarray*.
