...
QGIS recipes are available on conda-forge and work across all major distributions. This is a reliable and preferred method to setup a working environment for PyQGIS Development. You not only get all the Python bindings, but also a full QGIS installation with the user interface. Please follow the [Using QGIS from Conda](https://gisunchained.wordpress.com/2019/05/29/using-qgis-from-conda/) guide by Alexander Neto. 

1. Open a Terminal and run the following command to create a new conda environment.
...
2. Activate the newly created environemnt.
...
3. To install the `QGIS-LTR` version from Conda, you must specify the full version name. Search the `conda-forge` repository for available versions. The following command searches for all available packages for the LTR version 3.40 for your operating system.
...
4. Pick the version from the list to install.
...
5. The full QGIS software, along with all the Python bindings is now installed on your system. To launch QGIS, just type `qgis` in the terminal. You can use this version of QGIS for your GIS tasks. Remember to always activate the conda environment before typing the `qgis` command.
