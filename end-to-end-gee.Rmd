...
A technique applied to a time series for removal of the fine-grained variation between time steps is known as Smoothing. For moving window average smoothing, a temporal window *in this case '30 days'* is set to grab all the values in the window and compute the mean. So, the resulting graph reduces the sharp peaks and valleys due computed continuous value. 

> Note: Moving window max smoothing can also be done depending on the use-case, which gets the maximum temperature in the temporal window to generate a continuous value.
...
```{r echo=FALSE, fig.align='center', out.width='100%', fig.cap='Moving Window Average Smooth'}
