...
In this exercise, you will digitize all water bodies in the city of Banglore, India. This city has experienced urbanization at a rapid phase, and due to this development, some waterbodies have been compromised as urban buildings. We will use the georeferenced scanned map from the previous exercise to map all the water bodies in 1924 and label their current status. Finally, we will create a vector layer and assign attributes to them, whether healthy, lost, or partially lost.
...
2. This project contains the ``OSM Standard`` basemap and the ``Banglore_1924_modified`` georeferenced scanned map. To digitize the waterbodies, let us create a new vector layer. Click on the *New GeoPackage Layer...* icon from the toolbar. 
...
4. In the new layer, let us add some basic fields. First, let us add the name field. Under the *New Field* section, in *Name* enter ``name``, in *Type* choose ``Text Data``, and in *Maximum length*, enter ``50``. Now click *Add to Fields List* button. 
...
7. Let us inspect the attribute table of the new layer. Right-click on the ``banglore_lakes`` layer and click *Open Attribute Table*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_07.png')
```

8. In the banglore_lakes attribute table, there are three fields. ``fid`` will be unique and autogenerated, ``name`` and ``status`` must be fed while digitizing the waterbodies. Close the attribute table. 
...
10. In the *Layer Properties* dialog, choose *Attribute Form*. Under *Fields*, select *status*. Choose the *Widget Type* as ``Value Map``. Value Map allows us to create a drop-down while adding field values for each feature(polygon).
...
11. Enter *Value* as ``1``, *Description* as ``Healthy``. *Value* as ``2``, *Description* as ``Partially Lost``, and *Value* as ``3``, *Description* as ``Lost``. The status field will be set as a drop-down with these three values.
...
13. In the QGIS main canvas, before editing, let us enable the snapping toolbar. This toolbar will help select the nearby vertex corners and avoid invalid geometries. Right-click on the toolbar and check the *Snapping Toolbar* to enable it.
...
15. To start digitizing, select the ``banglore_lakes`` layer. Click the *Toggle editing* and the *Add Polygon Feature* button. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_15.png')
```

16. Enable snap setting by clicking *Enable Snapping* button. Zoom to any water body region. Starting from an edge, keep on adding vertices using left-click. Once the polygon is fully covered, right-click to complete the feature digitization. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_16.png')
```

17. Now the *Feature Attributes* dialog will popup to get the attributes value. By referring to the ``OSM Standard`` basemap, enter the *name* and select the *status* of the waterbody. Click *Ok*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_17.png')
```

18. Now the polygons show the fully digitized feature.
...
21. Let us inspect the attribute table of the digitized layer. Now right-click on the ``banglore_lakes`` layer and click *Open Attribute Table*. 
...
1. Style the layer based on the *status* column. This column has categorical values. We can use this to differentiate the different statuses of each waterbody. 
