...
# 14. Writing Standalone Python Scripts
...
## Create a New Toolbar with Dropdown Menu
...
## Standalone Scripts with Processing Algorithms

You can access all processing algorithms from standalone PyQGIS scripts. Processing is implemented as a core **plugin** in QGIS. So you need to make sure the path to core plugin folder is included in the PYTHONPATH environment variable. Our recommended [Windows](#windows-configuration) and [Mac](#macos-configuration) configurations described in the previous sections already include this. So all you need to do is to import the `processing` module and initialize it.

The script below shows how to use the `qgis:listuniquevalues` algorithm to get unique values in one of the fields of a vector layer. You can save it as `unique_values_processing.py` and run it via a Windows Batch file or Shell script as described earlier.


```{python eval=FALSE, code=readLines('code/pyqgis/unique_values_processing.py')}
```
