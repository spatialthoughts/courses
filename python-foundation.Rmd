...
6. Now we are ready to install the required packages using the `conda install` command. We can now install other required packages for this class. Run the command below to install `geopandas`, `geopy`, `rasterio`, `matplotlib` and `jupyterlab` packages. We use the [conda-forge](https://conda-forge.org/docs/user/introduction.html) channel which has a broader range of packages available than the default channel.

```
conda install --channel conda-forge geopandas geopy rasterio matplotlib jupyterlab -y
```
