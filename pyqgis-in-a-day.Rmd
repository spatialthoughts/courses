...
## 8.1 Actions Deep Dive

# Learn more

Learn about actions in the following video where I explain some powerful use cases.

[![Video](images/pyqgis/actions_deep_dive.png){width="70%"}](https://www.youtube.com/watch?v=8rOe-9A84O8){target="_blank"}

[View the Presentation &#8599;](https://docs.google.com/presentation/d/12J5XaJ8vVNvx8AfYywAk76rrxBxgn8Qyht6ouyB2_Yg/edit?usp=sharing){target="_blank"}
...
# 10. Understanding Python Iterators
...
# 12. Writing Standalone Python Scripts
...
## 13.1 Writing a Python Class

We have sen how to use a class from PyQt or PyQGIS libraries. But we have not written a new class. To write a Processing Script, we must write a new class. A new class is defined using the word `class`. All classes have a function called` __init__()`, which is always executed when a new object is being created. There is also the keyword `self` which refers to the current instance of the class. Here's the code to create a class called `Car` demonstrating the example we covered in the [Classes and Objects](https://docs.google.com/presentation/d/1b4i1nEDR_uKJVu0IIqjoqrxeFD2a-tnj88JRXQW0JiA/edit?usp=sharing) presentation.

```{python eval=FALSE, code=readLines('code/pyqgis/car.py')}
```

As we saw, all PyQGIS and PyQt inherit from other classes. Let's see them in action.

## 13.2 Writing a Processing Script
...
## 13.3 Simplifying Processing Scripts
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/pyqgis/pluginminimalshow1.png')
```

Now the *Hello from Plugin* message is displayed. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/pyqgis/pluginminimalshow2.png')
```

## Exercise 6

Take the minimal plugin template from above and build a new plugin called **Show Time**. This plugin will add a button to the Plugin Toolbar which will display the current time when clicked. The code to add such a button is already covered in the [Add a button to a toolbar](#add-a-button-to-a-toolbar) section.

## 14.2 Processing Plugin
...
* [Deploying a Customized Version of QGIS](https://github.com/haubourg/custom-osgeo4w-qgis) by Régis Haubourg 
