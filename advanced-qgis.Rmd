...
8. Double click on ``Create spatial index`` algorithm in model canvas and click … in *Dependencies.* 
...
11. In the *Description* type **Extract by location (No Spatial Index)**. Choose *Algorithm Output* in *Extract features from* and select **"Grid" from algorithm "Create grid"**. Click … in *Dependencies*.
...
13. Similarly in *Extract by location* using *Spatial Index* change the description to **Extract by location (Spatial Index)**.  Then open the *Count points in polygon* algorithm. 
...
15. In the *Expression Dialog*, we need to select output from any one of the *Extract by location* algorithms based on the user condition of ``@usespatialindex``. We can use the ``if`` condition to return a layer based on user input. Use the below formula. Click **OK**. 
...
22. Now switch to *Parameters*, uncheck the *Use Spatial Index* and click **Run**.  tab. 
...
6. Now in *Browser*, under ``martime_piracy.gpkg`` the project is saved. 
