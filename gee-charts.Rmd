...
```{js eval=FALSE, code=readLines('code/gee_charts/01-Time_Series_Charts/01c_Time_Series_(exercise)')}
```
...
- `series.visibleInLegend`: Set whether a particular series label is visible in the legend.
...
```{js eval=FALSE, code=readLines('code/gee_charts/01-Time_Series_Charts/03c_Time_Series_By_Region_(exercise)')}
```
...
- `interpolateNulls`: Set whether to fill missing (i.e masked) time-series values
...
```{js eval=FALSE, code=readLines('code/gee_charts/01-Time_Series_Charts/04c_Time_Series_By_Year_(exercise)')}
```
