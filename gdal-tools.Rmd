...
> Note: We could have done this operation in a  single step using the command `gdalbuildvrt merged.vrt *.hgt`. However, some versions of GDAL on Windows do not expand the `*` wildcard correctly and the command results in an error. It is recommended to use a file list instead of wildcards with GDAL commands on Windows to avoid unexpeted results.[[reference](https://github.com/OSGeo/gdal/issues/1749)]
...
> This operation requires a lot of processing and may take a few minutes. To improve the performance, you can add the configuration option `--config GDAL_CACHEMAX 512` to allow the command to use 512 MB of RAM for reading/writing data.
...
Now that we have all the input layers converted, we can generate proximity rasters. These are also known as Euclidean distance rasters - where each pixel in the output raster represents the distance to the nearest pixel in the input raster. This resulting raster can be then used to determine suitable areas which are within certain distance from the input.
...
For raster algebra operations, GDAL provides a raster calculator in form of `gdal_calc.py`. The input rasters are specified using using any letters from A-Z. These letters can be then referenced in the expression. The expression is specifited using the `--calc` option and it supports NumPy syntax and [functions](https://numpy.org/doc/stable/reference/routines.math.html#). The following commands apply the expressions to take the continuous values from the input rasters and create re-classified rasters with discrete values.
...
Perform a *Weighted* Multi-Criteria Overlay Analysis where different criteria are assigned weights on their relative importance. Let's assume that proximity to cycling routes is not as important as the other two factors in the final decision. We can give more important to other criteria as below.
