...
This is one the key features in QGIS 3.14 and later versions. We can use this to check for a condition and determine which task is to be executed after that. This can be easily understood as If statements from any programming language say *C++, Python, ...* 

Let us develop a Conditional branch using the Piracy Hexbin project. We will add a new checkbox input that allows the user to specify whether to use a spatial index in the model. The conditional branch will allow us to take different actions based on the user selection.
...
```
6. Search **usespatialindex**  and double click on it. Click OK.
...
17.  Model execution will be successful and also the spatial indexing has enhanced the model performance by reducing the time. 
