...
A useful tool called `gdallocationinfo` comes with the GDAL suite and can do point lookups from the raster at one or more pairs of coordinates. Combined with GDAL's capabilities to stream data from cloud datasets with the [Virtual File System](https://gdal.org/user/virtual_file_systems.html), you can efficiently lookup pixel values from without having to load the entire raster into memory.

For example, we have a large (8GB) single-band Cloud-Optimized GeoTiff (COG) file on a Google Cloud Storage bucket. This file is a global mosaic of VIIRS Nighttime Lights for the year 2021 and the pixel values are the radiance values. To access this file, we use the `/vsigs/` (Google Cloud Storage files) handler and provide the path in form of `/vgigs/<bucket_name>/<file_name>`. This particular file is public, so we add the config option `--config GS_NO_SIGN_REQUEST YES`.
...
```
gdallocationinfo -valonly /vsigs/spatialthoughts-public-data/viirs_ntl_2021_global.tif -wgs84 77.5946 12.9716 --config GS_NO_SIGN_REQUEST YES
```
...
```
cat coordinates.txt | gdallocationinfo /vsigs/spatialthoughts-public-data/viirs_ntl_2021_global.tif \
...
```
<Report pixel="57351" line="37185">
  <BandReport band="1">
    <Value>31.109167098999</Value>
  </BandReport>
</Report>
<Report pixel="57242" line="33421">
  <BandReport band="1">
    <Value>58.9608345031738</Value>
  </BandReport>
</Report>
```
