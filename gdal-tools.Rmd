...
*Windows*
```
python %CONDA_PREFIX%\Scripts\gdal_merge.py  -o rgb.tif -separate -co PHOTOMETRIC=RGB -co COMPRESS=DEFLATE RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B4.tif RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B3.tif RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B2.tif
```

*Mac/Linux*
```
python $CONDA_PREFIX/bin/gdal_merge.py  -o rgb.tif -separate -co PHOTOMETRIC=RGB -co COMPRESS=DEFLATE RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B4.tif RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B3.tif RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B2.tif
```
...
```
gdal_calc.py -A RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B5.TIF -B RT_LC08_L1TP_137042_20190920_20190926_01_T1_2019-09-20_B4.TIF --outfile ndvi.tif --calc="(A-B)/(A+B)" --NoDataValue=-999
```
...
The source data layers are provided in the `multicriteria.gpkg`. This file is in the `multicriteria_overlay` folder. Switch to the folder using the `cd` command.

```
cd multicriteria_overlay
```

The step in the overlay analysis is to convert each vector data layer to raster. This is accomplished using the `gdal_rasterize` command. Let's check the content of the data layers.
...
## 3.5 Building a Processing Pipeline

Once you know the individual commands to be run for each step of your procesisng chain, combining them to be run as a pipeline is very easy. You can create a script that executes each step one after the other. When you run the script, you will get the output without any manual intervention.

Below is an example of a script that runs the multicriteria overlay analysis we just saw. Save the content below in a file in the same directory as `multicriteria.gpkg`. The file name will be different depending on your operating system.

* Windows: `run_multicriteria.bat`
* Mac/Linux: `run_multicriteria.sh`


```{bash eval=FALSE, code=readLines('code/gdal/run_multicriteria.sh')}
```

Then from the same directory, you can launch the script as below.

*Windows*
```
run_multicriteria.bat
```

*Mac/Linux*
```
bash run_multicriteria.sh
```
...
Let's say we have a large GeoTIFF file `aoi.tif` and want to split it into tiles of 5000 pixels. 
...
Keyhole Markup Language (KML) is a XML-based file format primarily used by Google Earth. Often time, GIS users want to export their data to KML to visualize them in Google Earth. Other times, you may want to extract data from KML files or convert them into other spatial formats used in GIS. The [GDAL KML Driver](https://gdal.org/drivers/vector/kml.html) can both read and write KML files and provides many options to make the conversion compatible.

### Exporting Data to KML files

Let's take the `metro_stations` layer from the `spatial_query.gpkg` file in your data package and export it to KML.
...
OGR also comes with support for [LIBKML Driver](https://gdal.org/drivers/vector/libkml.html#vector-libkml). This driver supports many more options and allows you to create fully featured KMLs. If your GDAL version supports it, you can use it instead of the KML driver. To specify the name field, the LIBKML driver uses an environment variable called `LIBKML_NAME_FIELD` that can be specified with the `--config` option
...
Let's extract the `Paths` layer from this KML file. The default options create many unwanted fields in the output. We can select a subset of the input fields using the `-select` option.
