...
> Tip: If you want to calculate the great-circle distance assuming the earth is a sphere, you can set the ellipsoid to 'sphere'. All valid ellipsoids can be found by calling `QgsEllipsoidUtils.acronyms()`

Now our object `d` is capable of performing ellipspodal distance computations. Browsing through the available methods in the `QgsDistanceArea` class, we can see a `measureLine()` method. This method takes a list [QgsPointXY](https://qgis.org/pyqgis/master/core/QgsPointXY.html) objects. We can create these objects from our coordinate pairs and pass them on to the `measureLine()` method to get the distane.
...
Putting it all together, below is the complete code to caluclate the distance between a pair of coordinates using PyQGIS. When you run the code in the Python Console of QGIS, all the PyQGIS classes are already imported. If you are running this code from a script or a plugin, you must explicitely import the `QgsDistanceArea` class.
...
Let's say you want to make a stop at Las Vegas on the way fromo San Francisco to New York. Calculate the total ellipsoidal distance considering a stop at las vegas.
