...
* **Why the requirement for a Cloud Project?** Previously - Earth Engine was a separate Google service with it's own systems and was governed by its own Terms of Service. It is now integrated into Google Cloud Platform and is one of hundreds of other cloud offerings. This means that the usage of GEE is now governed by the *Google Cloud Terms of Service* and it uses the standard cloud methods for access, logging, billing etc. It helps Earth Engine team offer better integration with other Google Cloud offerings. Using Google Cloud requires a *Project* - which is just a workspace where you can enable different services.
