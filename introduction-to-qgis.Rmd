...
Access the Presentation [Google Slides &#8599;](https://docs.google.com/presentation/d/1Lw0NQlLQXUSW1Mf1rzy9pLb_m2wxSUHQDxF-vjPTBaU/edit?usp=sharing){target="_blank"} [PDF &#8599;](https://github.com/spatialthoughts/courses/releases/download/presentations/Introduction_to_QGIS_Introduction_and_Course_Overview.pdf)
...
The course is accompanied by a video covering the all the modules. This video is recorded from our live instructor-led class and is edited to make them easier to consume for self-study. We have 2 versions of the videos:
...
We will be using the following plugins during the course. From the *Plugins* menu, choose *Manage and Install Plugins...*. Under the *All* tab, search for the plugin name and click on the *Install Plugin* button to install it.
...
We have now finished the second part of this exercise. Your output should match the contents of the `Earthquakes_Checkpoint2.qgz` file in the `solutions` folder.
...
> Note: If you do not see any results after searching, expand the **Projected** section.
...
8. Next, we will change the appearance of the label text. Switch to the *Text* tab. Change the *Size* to **6** and set the *Color* to white.

> Note: The exact font size will depend on your system and screen resolution. You may adjust it to ensure labels are legible.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_8.png')
```

9. A useful labeling technique is to add a background to the labels to improve legibility. Switch to the *Background* tab and enable *Draw background*. Set the *Color* to black. Also, set the *Size X* and *Size Y* of *Buffer* to be **1** point.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_9.png')
```

10. Scroll down and set the *Radius X,Y* to **5**.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_10.png')
```

11. To attach the labels to each symbol, you can use a leader line. Switch to the *Callouts* tab and check *Draw callouts*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_11.png')
```

12. As we have only a few labels, we can adjust their placement manually to fit the layout better. Close the *Layer Styling Panel* and locate the *Label Toolbar*. If you do not see the toolbar, see the instructions at [Enable Required Toolbars](#enable-required-toolbars) to activate it.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_12.png')
```

13. Once the *Label Toolbar* is activated, you will see new tools. Select the *Move a Label, Diagram, or Callout* tool.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_13.png')
```

14. Click on any of the labels. The first time you do this, you will be prompted with an *Auxiliary Storage: Choose Primary Key* dialog. This is used to store the position of the labels for each label. You can choose any field containing unique values. In our case, the default value **fid** is fine. Click *OK*.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_14.png')
```

15. Click on the label again to start moving it. Drag your mouse to the new label location and click again to move the label.
...
16. Similarly, move other labels to appropriate locations. Once you are satisfied, save your work.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part3_16.png')
```
...
8. Drag a rectangle at the top. On the right-hand side *Item Properties* panel, enter **10 Largest Earthquakes (2000-2020)** as the text. Click on the *Font* button under *Appearance* and set the *Font Size* to **24**. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part4_8.png')
```

9. Scroll down and select **Center** as the *Horizontal alignment* and **Middle** as the *Vertical Alignment* to center align the label. We will now add a legend to the map. Go to *Add Item &rarr; Add Legend*. Draw a rectangle at an empty region on the map.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part4_9.png')
```

10. Once the legend is added, Select **Manual** option for the *Legend Items*. This will allow us to customize the items in the legend. Select the ``ne_10m_land`` layer and click the *Remove selected item(s) from legend*. 

> Note: In older versions of QGIS, you will need un-check the *Auto update* box.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/earthquakes_part4_10.png')
```

11. You can also edit the layer name to be more suitable for the map. Select the ``gem_active_faults_harmonized`` layer and click the *Edit selected item properties* button. Change the name of the layer to **Faults**. Similarly, change the ``significant_earthquakes_2000_2020`` layer name to **All Significant Earthquakes** and  ``large_earthquakes`` to **Deaths**.
...
[Google Slides &#8599;](https://docs.google.com/presentation/d/1YJfWrjSDeriYJMsA6bmVi4zZmzLmGOo5dm13Lr5JaiI/edit?usp=sharing){target="_blank"} [PDF &#8599;](https://github.com/spatialthoughts/courses/releases/download/presentations/Introduction_to_QGIS_Joins.pdf)
...
3. You may be prompted to *Select Transformation for nynta2010*. The source shapefile is in the `EPSG:2263 NAD83 / New York Long Island (ftUS)` projection whereas the default projection in QGIS is `EPSG:4326 - WGS84`. It does not matter in this case - so you can choose the first option and click *OK*. In some versions of QGIS, you may see a warning about the transformation. You may ignore this and close the message.
...
13. Once the filter is applied, right-click the ``New_York_City_Population_By_Neighborhood _Tabulation_Areas`` layer and select *Show feature count*. This will show the active features present in the table. Do the same for the ``nynta2010`` layer and ensure that the number of features in that layer match with that of the table.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/population_part1_13.png')
```

14. Now we are ready to do the table join. Open **Processing &rarr; Toolbox** from the main menu at the top. Search and locate the algorithm **Vector general &rarr; Join attributes by field value** and double-click to launch it.
...
22. In the *Field Calculator* dialog, enter ``Density`` as the *Field Name*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/population_part1_22.png')
```

23. We will now build the expression to calculate population density. From the function groups next to the *Expression* panel, expand the *Fields and Values* section. Double-click the ``Population`` field to add it to the expression editor. Note that fields are referred using double-quotes (") in QGIS. Select the **/** button to enter the division operator and then click the ``Shape_Area`` field to enter it. You may also type the expression instead of picking the values from the dialog. The final expression should look like below. 
...
24. This will give us the population density in persons per square feet. A more appropriate unit for population density is persons per square miles. Let's convert the value to miles using the conversion factor or *1 mile = 5280 ft*. Change the expression as shown below. You will see the result of the expression in the *Preview* below.
...
25. Once done, select *Save to File*. Name the output as `nynta_population_density.gpkg` and click *Run*.
...
27. We don't need the other layers in our project. Hold the *Ctrl* key and select all layers except `nynta_population_density`. Right-click and select *Remove Layer...*.
...
[Google Slides &#8599;](https://docs.google.com/presentation/d/1cB6sF_Lo3w2YKhhxSPZdKKaYVT2ID91Jcvf-E0P4MUk/edit?usp=sharing){target="_blank"} [PDF &#8599;](https://github.com/spatialthoughts/courses/releases/download/presentations/Introduction_to_QGIS_Data_Normalization.pdf)
...
3. As we want to map the population density, choose ``Density`` as the *Value*. At this step, the map will disappear. Do not worry as it will be re-rendered with the chosen classes when we apply the classification. 
...
We have now finished the second part of this exercise. Your output should match the contents of the `NYC_Population_Density_Checkpoint2.qgz` file in the `solutions` folder.
...
1. Open QGIS. We will use a plugin called **QuickMapServices** to load a basemap. If you haven't already, you can install it using the instructions from the [Install Required Plugins](#install-required-plugins) section. Once installed, go to *Web &rarr; QuickMapServices* menu. You will see some map providers and available basemaps. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part1_1.png')
```

2. For our current task, we will use a basemap based on OpenStreetMap data. Since we need to locate the features in the scanned maps let's add the *OSM Standard*. Click on the **Web &rarr; QuickMapServices &rarr; OSM &rarr; OSM Standard**. 
...
3. Now in the canvas, the basemap will be loaded. This map is georeferenced and projected in **EPSG:3857** CRS. This information can be viewed on the bottom-right of the QGIS, where the project CRS will be updated. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part1_3.png')
```

4. You can use QGIS's built-in Geocoder to search and zoom to a place. Select the locator bar in the bottom left corner and enter the search for a place using the prefix **>**. i.e. If you want to search for the city of Bengaluru, enter **> Bengaluru**. Make sure there is a space between the > character and the search term. From the results, click on the first one to zoom to the location.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part1_4.png')
```

5. The canvas will now be zoomed and centered on the chosen location.
...
Load the **Google Satellite** basemap by Google. This is a useful basemap with high-resolution satellite imagery from Google Maps.
...
1. Click on the **Layer &rarr; Georeferencer** from the menu-bar to open the georeferencing tool.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_1.png')
```

2. The *Georeferencer* panel will open. The default setting is for the Georeferencer window to be docked in the main QGIS application. If you prefer, you can un-dock it by dragging the panel from the title. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_2.png')
```

3.  In the *Georeferencer* window, click on the **Open Raster..** button from the ribbon to load the scanned map. Browse to the ``Bangalore_1924.png`` file in your data package and click *open*.
...
> If the docking does not work - you can enable docking settings from *(Mac)* QGIS &rarr; Preferences &rarr; Configure Georeferencer or *(Windows)* Georeferencer &rarr; Configure Georeferencer. Check the *Show Georeferencer window docked* box to dock the georeferencer.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_5.png')
```

6. Now we start collecting GCPs. Locate a feature that is visible in both the scanned map and the basemap. Click on the *Add Point* button. 

> Note: You can use the *Space Bar* Key to toggle between moving the map and adding GCPs.
...
10. Let's view the points in a tabular format. If you had docked the window, detach it to a separate window. Go to **View &rarr; Panels &rarr; GCP table** to open the *GCP table* panel.
...
12. In the *Transformation Settings* dialog, choose **Polynomial 2** as *Transformation type*. Choose **Nearest Neighbour** as *Resampling method* and *EPSG:3857* as *Target SRS*. Click on the *...* in *Output raster* and save the file as ``Banglore_1924_modified.tif``. Check the *Raster creation options* and choose **High compression** *Profile*. 

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_12.png')
```

13. Finally, un-check the *Set target resolution* option, and ensure  the *Save GCP points* and *Load in QGIS when done* boxes are checked. Click *OK* to save this setting and close the settings dialog. 

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_13.png')
```

14. Note the *Residual (pixels)* columns will not display the error for each GCPs based on the chosen transformation algorithm. Lower error means the transformation would be able to accurately assign the chosen coordinate to the pixel.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_14.png')
```

15. If you have a GCP with high residual, you can use the *Move point* to adjust the point's position or *Delete point* to remove it. Before georeferencing, a minimum of six GCPs should be available.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_15.png')
```

16. Repeat this process till you are satisfied with the the total mean error and the residuals.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_16.png')
```

17. Click on the *Start Georeferencing* button to georeference the scanned map. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_17.png')
```

18. In the main QGIS canvas, the georeferenced layer ``Banglore_1924_modified`` will be added and overlayed on the basemap. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_18.png')
```
...
## 3.3 Georeferencing Resources

**Online repositories of scanned maps**

- [David Rumsey Map Collection](https://www.davidrumsey.com/)
- [Hipkiss' Scanned Old Maps](https://www.hipkiss.org/scannedmaps/index.html)
- [Wikimedia Old Maps](https://commons.wikimedia.org/wiki/Category:Old_maps)

**Georeferencing Tools**

- [Map Warper](https://mapwarper.net/): Free, open-source online tool for georeferencing historical maps and overlaying them on modern maps.
- [QGIS AI Georeferencer](https://buntinglabs.com/tools/ai-georeferencer): Paid AI-powered georeferencing tool for aerial imagery.
...
1. Go to *Project &rarr; Open* and browse to the data package. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_01.png')
```

2. Select the ``Digitizing.qgz`` project and click *Open*.

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_02.png')
```

3. This project contains the ``OSM Standard`` basemap and the ``Banglore_1924_modified`` georeferenced scanned map. To digitize the waterbodies, let's create a new vector layer. Click on the *Layer &rarr; Create Layer &rarr;  New GeoPackage Layer...* from the menubar. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_03.png')
```

4. In the *New GeoPackage Layer* dialog, click on the *...* next to *Database* and browse to the project location. Enter the file name as ``banglore_lakes`` and click *OK*. Now the *Table name* will be auto-populated as ``banglore_lakes``. Choose ``MultiPolygon`` as *Geometry type*. Let the CRS be in default ``EPSG:4326`` projection.  

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_04.png')
```

5. In the new layer, let's add some basic fields. First, let's add the name field. Under the *New Field* section, in *Name* enter ``name``, in *Type* choose ``Text``, and in *Maximum length*, enter ``50``. Now click *Add to Fields List* button. 

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_05.png')
```

6. The *Fields List* section will get updated. Similarly, add a ``status`` filed with *Type* as ``Integer (32 bit)``. Click *Add to Fields List* button.

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_06.png')
```

7. The *Fields List* should now show both the fields. Click *OK* to create the layer.

```{r echo=FALSE, fig.align='center', out.width='60%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_07.png')
```

8. Now the ``banglore_lakes`` layer will be added to the *Layers* tab.  

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_08.png')
```

9. let's inspect the attribute table of the new layer. Right-click on the ``banglore_lakes`` layer and click *Open Attribute Table*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_09.png')
```

10. You will see that the attribute table is empty. There are 3 three fields for the layer. The *name* and *status* are the ones we defined and must be entered while digitizing the waterbodies.  The *fid* field is an autogenerated field which is required by the GeoPackage format. Close the attribute table. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_10.png')
```

11. Again right-click on ``banglore_lakes`` layer and click *Properties* to open the properties dialog. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_11.png')
```

12. In the *Layer Properties* dialog, choose *Attribute Form*. Under *Fields*, select *status*. Choose the *Widget Type* as ``Value Map``. Value Map allows us to create a drop-down menu that allows you to pick from a predefined set of values.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_12.png')
```

13. The status field will be set as a drop-down with these three values. Enter the value and description as follows 
...
```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_13.png')
```

14. We require this field's input must be given for every feature, so let's make this a mandatory field. Under *Constraints* check *Not null* and *Enforce not null constraint*. Click *OK* to save the changes and close the properties dialog. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_14.png')
```
...
1. Before we start digitizing, make sure you have enabled the *Snapping Toolbar*. See the instructions at [Enable Required Toolbars](#enable-required-toolbars) if you need to enable it.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_1.png')
```

2. Enable snap setting by clicking *Enable Snapping* button. Zoom to any part of the map containing a waterbody. Toggle the visibility of the ``Banglore_1924_modified`` layer and check if the lake exists in the current basemap. This will be helpful in entering the attribute of the lake you will digitize.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_2.png')
```

3. Turn the ``Banglore_1924_modified`` on. Select the ``banglore_lakes`` layer and start digitizing the waterbody. Click the *Toggle editing* followed by the *Add Polygon Feature* button.  Starting from an edge, keep on adding vertices using left-click. Once the polygon is fully digitized, right-click to complete it.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_3.png')
```

4. Now you will be prompted by the *Feature Attributes* dialog to enter the attributes values for the feature you just created. By referring to the ``OSM Standard`` basemap, enter the *name* and select the *status* of the waterbody. Click *OK*

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_4.png')
```

5. Now the polygon show the fully digitized feature.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_5.png')
```

6. Zoom to another waterbody and digitize with the same process. If the waterbody name is unavailable in both basemap and scanned map, leave the *name* field empty and select the *status* of the waterbody. Click *OK*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_6.png')
```

7. Digitize all the available waterbodies. Once completed, click on the *Save Layer Edits* button and turn off the editing mode by pressing the *Toggle Editing* button.  

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_7.png')
```

8. let's inspect the attribute table of the digitized layer. Now right-click on the ``banglore_lakes`` layer and click *Open Attribute Table*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_8.png')
```

9. The Attribute Table contains ``24`` digitized features. You can note the *fid* column is auto-populated with a unique value for each record. Close the attribute table. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part2_9.png')
```

10. Now let's save the project, click *Project &rarr; Save*. 
...
[Google Slides &#8599;](https://docs.google.com/presentation/d/1hiVXo82VnRHrQBroxzrByYod8CeNvsB1iVwy6q62mz8/edit?usp=sharing){target="_blank"} [PDF &#8599;](https://github.com/spatialthoughts/courses/releases/download/presentations/Introduction_to_QGIS_Introduction_to_OpenStreetMap.pdf)
...
2. Now, we will query the OpenStreetMap database to get a vector layer of railway stations in the city. We will use the **QuickOSM** plugin for this task. If you haven't already, you can install it using the instructions from the [Install Required Plugins](#install-required-plugins) section. Once installed, go to **Vector &rarr; QuickOSM &rarr; QuickOSM**. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/geoprocessing_part1_2.png')
```

3. Select the *Quick query* tab. OpenStreetMap uses a tagging system to record properties of physical features. The tags and the key/value pairs are described in [OpenStreetMap wiki](https://wiki.openstreetmap.org/wiki/Map_Features). The railway stations are tagged with ``railway=station``. Enter ``railway`` as the *Key*, ``station`` as the *Value* and *Bangalore* as the geographic filter *In*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/geoprocessing_part1_3.png')
```

4. Expand the *Advanced* section and check only the *Node* and *Points* boxes. Click *Run Query*.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/geoprocessing_part1_4.png')
```

5. Once the query finishes, you will see a new layer, ``railway_station_Bangalore`` loaded in the canvas. The city has railway stations operated by two different agencies - one that operates the intercity trains and other that runs the metro service. We can apply a filter to select only the metro stations. Right-click the layer and select *Filter*.
...
6.  In the *Query Builder*, enter the following expression to select the stations operated by the **Bangalore Metro Rail Corporation Limited** - the agency that operates the metro network in the city. Click *OK*.
...
7. Now, the map will update to show all the operational metro stations in the city. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/geoprocessing_part1_7.png')
```

8. Go to the *Symbology* panel and change the style to **Single Symbol**.
...
## Concept: Gridded Population Datasets

[![View Presentation](images/introduction_to_qgis/gridded_population_presentation.png){width="400px"}](https://docs.google.com/presentation/d/1ybJvCXRFfB2yQfPS9VE_pjTRDjQbX_PRcutpo16ksYI/edit?usp=sharing){target="_blank"}

[Google Slides &#8599;](https://docs.google.com/presentation/d/1ybJvCXRFfB2yQfPS9VE_pjTRDjQbX_PRcutpo16ksYI/edit?usp=sharing){target="_blank"} [PDF &#8599;](https://github.com/spatialthoughts/courses/releases/download/presentations/Introduction_to_QGIS_Gridded_Population_Datasets.pdf)
...
* Land boundaries: Made with Natural Earth. Free vector and raster map data. Downloaded from https://www.naturalearthdata.com/.
* Earthquake Faults: Styron, Richard, and Marco Pagani. "[The GEM Global Active Faults Database. (GEM GAF-DB)](https://www.globalquakemodel.org/product/active-faults-database)" Earthquake Spectra, vol. 36, no. 1_suppl, Oct. 2020, pp. 160–180, doi:10.1177/8755293020944182. Downloaded from https://github.com/cossatot/gem-global-active-faults
* Significant Earthquakes: National Centers for Environmental Information / World Data Service (NGDC/WDS): The significant Earthquake Database. National Centers for Environmental Information, NOAA. doi:10.7289/V5TD9V7K. Downloaded from https://www.ngdc.noaa.gov/hazel/view/hazards/earthquake/search
* 2010 Neighborhood Tabulation Areas (NTAs), Department of City Planning (DCP), New York City. Downloaded from [NYC Open Data Portal](https://data.cityofnewyork.us/City-Government/2020-Neighborhood-Tabulation-Areas-NTAs-/9nt8-h7nd).
...
*Introduction to QGIS Course* by Ujaval Gandhi [www.spatialthoughts.com](https://spatialthoughts.com)
...
&copy; 2022 Spatial Thoughts [www.spatialthoughts.com](https://spatialthoughts.com)
