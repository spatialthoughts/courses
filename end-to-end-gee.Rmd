...
The preferred method for installing the Earth Engine Python API is via Anaconda. Follow this steps to install Anaconda and required libraries. 
...
2. Once installed, search for *Anaconda Prompt* launch it. Anaconda prompt is a command-line that makes sure that you can use anaconda and conda commands from the prompt, without having to change directories or your path. It is very similar to CMD prompt (windows) / Terminal (Mac Linux). 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/end_to_end_gee/conda2.png')
```

3. Once the prompt launches, it will be in the *base* environment. It is not best practice to install other libraries in base (i.e root) environment as it may cause an error if any dependency becomes outdated then anaconda needed to be reinstated. So a new working environment is created and all required library for the project is installed in it. Enter the below code to create a new environment called **ee**. 

>NOTE : To paste commands in prompt use "Shift + Insert" 
...
9. Once the library installs successfully, enter the below code to install ```jupyter lab```, it is a web-based user interface that can be used for executing python commands in a notebook format file. 
...
12. Enter the below code and click the *play* button, if the execution is successful all the libraries and their dependencies are installed successfully. 
