...
1. Create a folder named **dashboard** on your Desktop.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_dataviz/dashboard1.png')
```

2. Open your favorite text editor and create a file with the following content. By convention, we import `streamlit as st`. Then we can use the `st.title()` to display the title of our dashboard and `st.write()` to add a paragraph of text. Save the file in the **dashboard** folder on your desktop as `app.py`
