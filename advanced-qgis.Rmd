...
In the 3D view, you can hold the **Shift** key and drag your mouse to tilt the top-down view. You will see the map in 3D. You can also use the controls on the right-hand panel to tild,zoom and pan the view.
...
```{r echo=FALSE, fig.align='center'}
knitr::include_graphics('images/advanced_qgis/3d.gif')
```
...
We will work with a land parcels data layer provided by the City of San Francico. The goal of this exercise is to demonstrate the use of aggregate expression for on-the-fly computation when digitizing new features.
...
```{r echo=FALSE, fig.align='center'}
knitr::include_graphics('images/advanced_qgis/aggregate5.gif')
```

There are many differnt types of aggregates available. We can use an aggregate called **concatanate** to compute a comma-separated text of all feature ids. Go to the **Attribute Form** properties again and select ``parcels`` field. Enter the following expression as the **Default Value**.
...
```{r echo=FALSE, fig.align='center'}
knitr::include_graphics('images/advanced_qgis/aggregate7.gif')
```
