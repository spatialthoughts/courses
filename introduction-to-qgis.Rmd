...
This is an introductory course that covers QGIS from the very basics. You will learn to use QGIS for mapping, spatial data processing, and spatial analysis. This class is ideal for participants with a basic knowledge of GIS who want to learn how to use QGIS to carry out common GIS tasks.
...
This section is designed to help you get familiar with the basic workflow of importing data layers, applying symbology, adding labels and designing layouts for maps. We will take a text file containing historical records of earthquakes and turn it into an informative visualization like below.
...
In this exercise your going to georeference a old scanned map of Banglore, India in 1924. This map is possibly hand-drawn and has no coordinate markings on it so we have to use any other georeferenced map as basemap to locate the features and obtain coordinates. QGIS has a variety of basemaps you can use any of the available map for georeferencing. 
...
1. Open QGIS. We have to install a plugin to load diffrent basemaps. From the *Plugins* menu choose *Manage and Install Plugins...*.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part1_1.png')
```

2. The *Plugins* dialog contains all the available plugins in QGIS. Under the *All* tab, search for ``quickmapservices``. This plugin comes pre-installed but needs to be enabled. It has different basemaps that can be used based on the purpose. Check the box next to the *QuickMapServises* label. If you are prompted with *Upgrade Plugin* in the bottom right, click the button to upgrade and then click *Close*.
...
3. Now you will see a new *Web* menu added to the menubar. Click on it and expand the *QuickMapServices* menu. Depending upon your QGIS usage, you may or may not have all the basemaps. To enable all the basemap, click on the *Settings*. 
...
4. In the settings dialog, switch to the *More services* tab. Click on the ``Get contributed pack`` to download and enable all available basemaps. 
...
5. Now under *QuickMapServise* menu, you will get the latest available basemaps. The basemaps available here have different terms and conditions to be followed before displaying it in any applications. Hence use it with caution. 
...
6. The OpenStreetMap has different themed basemaps. Since we need to locate the features in the scanned maps let's add the *OSM Standard*. Click on the **Web &rarr; QuickMapServices &rarr; OSM &rarr; OSM Standard**. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part1_6.png')
```

7. Now in the canvas, the basemap will be loaded. This map is georeferenced and projected in **EPSG:3857**. This information can be viewed on the bottom-right of the QGIS, where the project CRS will be updated. 
...
Load the ``Openstreetmap tracks`` as a basemap. This layer contains the GPS traces that were tracked and made public. 

## 3.2 Loading a Scanned map and gereferencer tool tour.  

In the second session, we will load the scanned image and understand the georeferencing tools. 

1. Click on the **Raster &rarr; Georeferencer** from the menubar to open the georeferencing tool.
...
4. The scanned map will be loaded in the image pane. To collect the GCPs, let's close the table pane for now and dock the georeference to the QGIS main window. Click and drag the georeferencer title bar to the bottom of the canvas.
...
We have now finish the second part of this exercise. You should have loaded the scanned map and got a deep understanding of the georeferencer. 

## 3.3 Collect GCP and Georeference. 

In this final section, we will collect multiple ground control points and save the scanned map as a georeferenced TIF. 

1. Locate a feature both  in the scanned map and the basemap. Click on the *Add Point* button. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_1.png')
```

2. The *Enter Map Coordinates* dialog will appear. Click on the *From Map Canvas* button since we don't have the coordinates marking.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_2.png')
```


3. Now drop a point in the QGIS main canvas above the matching feature from the scanned map. This will auto-populate the coordinates and the projection. Click *OK* to close the dialog.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_3.png')
```


4. Similarly, choose a minimum of six-point. The best features to look for are rail-road intersections, building corners, city squares, or any other corner and edges. After choosing the points, click on the pop-out button in Georeferencing window to detach this tool from the main QGIS. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_4.png')
```


5. Now we can view the points in tabular format. Click on the **View &rarr; Panels &rarr; GCP table**. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_5.png')
```


6. The *GCP table* dialog will display the details of each point, the *Residual (pixels)* will denote the error magnitude created by each point during the georeferencing. Once the transformation is set, the residual error will be displayed. To assign transformation, click on the *Transformation Settings…* button.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_6.png')
```

7. In the *Transformation Settings* dialog, choose *Polynomial 2* as *Transformation type*. This type is known as rubber sheet transformation, as the scanned map will be adjusted for the GCPs chosen. Choose *Nearest Neighbour* as *Resampling method* and *EPSG:3857* as *Target SRS*. Click on the *...* in *Output raster* and save the file as ``Banglore_1924_modified.tif``. Raster files are huge, so compressing using a lossless technique is always recommended. Choose *LZW* in *Compression*. Finally, check the *Save GCP points* to edit it in the future for improvements, and check *Load in QGIS when done*. Click *OK* to save this setting and close the settings dialog. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_7.png')
```

8. The residual for each point will be calculated, and the mean error will be displayed at the bottom. To get a better-georeferenced image, the mean error should be minimum.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_8.png')
```


9. Locate the point with the highest residual. We use the *Move point* or *Delete point* to adjust the point position. Since we have more GCPs, let's delete the point with high residue. Before georeferencing, a minimum of six GCPs should be available.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_9.png')
```


10. The total mean error and the residual for each point are lowered considerably.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_10.png')
```

11. Click on the *Start Georeferencing* button to georeference the image. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_11.png')
```

12. In the main QGIS canvas, the georeferenced image  ``Banglore_1924_modified`` will be added and overlayed on the basemap. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part3_12.png')
```

We have now completed all steps and you should have a georeferenced image. You can load the image ``Georeferencing_final.qgz`` from solutions directory to verify. 

### 3.3.1 Assignment 

1. Load the GCPs collected and the ``Banglore_1924.png`` scanned map in the georeferencing tool. 

2. Adjust the points to reduce the mean error and create a more accurate map. 
