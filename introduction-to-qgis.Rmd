...
4. In the new layer, let's add some basic fields. First, let's add the name field. Under the *New Field* section, in *Name* enter ``name``, in *Type* choose ``Text``, and in *Maximum length*, enter ``50``. Now click *Add to Fields List* button. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_04.png')
```

5. The *Fields List* section will get updated. Similarly, add a ``status`` filed with *Type* as ``Integer (32 bit)``. Click *Add to Fields List* button.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_05.png')
```

6. After adding both the fields, click *OK* to close the dialog.   

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_06.png')
```

7. Now the ``banglore_lakes`` layer will be added to the *Layers* tab.  

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_07.png')
```

8. let's inspect the attribute table of the new layer. Right-click on the ``banglore_lakes`` layer and click *Open Attribute Table*. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_08.png')
```

9. In the ``bangalore_lakes`` attribute table, there are three fields. *fid*  is an integer field which is required by the GeoPackage format and is autogenerated. The *name* and *status* must be entered while digitizing the waterbodies. Close the attribute table. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_09.png')
```

10. Again right-click on ``banglore_lakes`` layer and click *Properties* to open the properties dialog. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_10.png')
```

11. In the *Layer Properties* dialog, choose *Attribute Form*. Under *Fields*, select *status*. Choose the *Widget Type* as ``Value Map``. Value Map allows us to create a drop-down menu that allows you to pick from a predefined set of values.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/digitizing_part1_11.png')
```

12. The status field will be set as a drop-down with these three values. Enter the value and description as follows 
...
13. We require this field's input must be given for every feature, so let's make this a mandatory field. Under *Constraints* check *Not null* and *Enforce not null constraint*. Click *OK* to save the changes and close the properties dialog. 
