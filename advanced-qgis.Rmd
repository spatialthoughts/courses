...
We start from the `GPS_Animation_Checkpoint2.qgz` file in the `solutions` folder. 

1. The `track_points` layer has the timestamp and positions. We need to find the cumulative distance at each of the points. This can be achieved by measuring the distance along the line in `tracks` layer from the first point to the current point. 
