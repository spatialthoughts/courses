...
Till this point in the course, we have used the Earth Engine Javascript API for all our analysis. Earth Engine also provides a Python API. The Python API is almost identical to the Javascript API. If you are a Python programmer, you prefer to use the Python API as it can integrate with your Python code. As you are able to use the Python API in your own environment (vs the Code Editor) - you get a lot more flexibility to automate as well as combine other analysis and visualization libraries with Earth Engine.

### Installation

The preferred method for installing the Earth Engine Python API is via Anaconda. Please look at [Python installation](https://developers.google.com/earth-engine/guides/python_install-conda) for step-by-step instructions. If you already have a conda environment setup, you can install the `earthengine-api` package.
...
```{r echo=FALSE, fig.align='center', out.width='100%', fig.cap='Creating a Continuous Rasteer Legend'}
