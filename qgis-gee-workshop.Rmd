...
This workshop requires QGIS LTR version 3.40 or above with Python 3.10 or above. If you have a QGIS version installed, check the bundled Python version by going to *QGIS Help menu &rarr; About*

For new installation, see the recommendation below.

* Windows/Linux users: Follow our [QGIS-LTR Installation Guide](install-qgis-ltr.html) for step-by-step instructions.
* Mac users: The official installers come with an older Python version which is not compatible with the Earth Engine API. We recommend installing QGIS using the new [Notarized QGIS Builds](https://github.com/m-kuhn/QGIS/releases/). You can also install the latest QGIS version using [MacPorts](https://qgis.org/resources/installation-guide/#macports).
