...
Your data folder contains a data layer ``national_highways_v2``. This contains an updated version of the ``national_highways`` layer extracted from the OpenStreetMap database. Use the **Detect Dataset Changes** algorithm from the Processing Toolbox and find out all road segments that have changed in the new version.

Hint: Use only the **osm_id** as the attribute for matching.
...
1. Add the ``karnataka_districts`` later from the ``karnataka.gpkg`` file. This layer contains the geometry of each district along with its name in the **DISTRICT** field.
...
Hint: You can use the algorithm **Extract by Expression** with the expression ``"NUMPOINTS" = maximum("NUMPOINTS")``. This expression will match only the feature where the value of the NUMPOINTS field is the highest.
...
Your can load the `Modeler_Checkpoint3.qgz` project contained in the ``maritime_piracy.gpkg` in the `solutions` folder.
...
1. It will be useful to add a label to the animation showing the date of the animation frame being displayed. We can use the *Title Decoration* to place that label. Go to **View &rarr; Decorations &rarr; Title Label Decorations.** Click the checkbox to enable it and then the **Insert an Expression** button.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/advanced_qgis/2d10.png')
```

2. Enter the following expression to display the year and click *OK*.
...
3. Select font size as **25**, set background color as **white.** In placement choose **Bottom Right.** Now click OK.

![[View Animated GIF &#8599;](images/advanced_qgis/2d12.gif){target="_blank"}](images/advanced_qgis/2d12.png)

4. Once the parameters are set accordingly, the date will displayed on the map. 


## 4.3 Exporting Animation
