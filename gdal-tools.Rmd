...
gdal_translate -of COG merged.vrt merged_cog.tif ^
  -co COMPRESS=DEFLATE -co PREDICTOR=2 -a_nodata -9999
```
...
> Note that JPEG is a lossy compression method and can cause edge artifacts for image mosaics. See [Supplement](#removing-jpeg-compression-artifacts) for techniques to mask the black portion and obtian clean edges.
...
We will create a geopackage file by merging all files in the directory.

```
ogrmerge.py -o earthquakes.gpkg *.geojson
```

If you get an error running the command, use the alternative syntax specified earlier to specify the full path to `ogrmerge.py`.
...
*Mac/Linux*
```
python $CONDA_PREFIX/bin/ogrmerge.py -o earthquakes.gpkg *.geojson
```
...
```
ogrmerge.py -o earthquakes.gpkg *.geojson -single -nln all_earthquakes -overwrite_ds
```
...
**Windows**
```
gdal_proximity.py existing_parking.tif existing_parking_proximity.tif ^
...
**Windows**
```
gdal_calc.py -A thefts.tif --outfile thefts_reclass.tif ^
  --calc="100*(A>20) + 50*(A>10)*(A<=20) + 10*(A<10)"

gdal_calc.py -A routes_proximity.tif --outfile routes_reclass.tif ^
...
**Windows**
```
gdal_calc.py -A thefts_reclass.tif -B routes_reclass_resampled.tif ^
...
Once you know the individual commands to be run for each step of your processing chain, combining them to be run as a pipeline is very easy. You can create a script that executes each step one after the other. When you run the script, you will get the output without any manual intervention.
...
# 5. Automating and Scheduling GDAL/OGR Jobs
...
Applying JPEG compression on aerial or drone imagery, sometimes you end up with edge artifacts. Since JPEG is a lossy compression algorithm, the black pixels values (0) are This is caused by no-data values (typically 0) being converted to non-zero values. This causes problems when you want to mosaic different tiles, or mask the black pixels. Fortunately, GDAL comes with a handy tool called [nearblack](https://gdal.org/programs/nearblack.html) that is designed to solve this problem. You can specify a tolerance value to remove edge pixels that may not be exactly 0. It scans the image inwards till it finds these near-black pixel value and masks it. Let's say we want to take the mosaic created in the [Moasic and Clip to AOI](#mosaic-and-clip-to-aoi) section and mask the black pixels. If we simply set 0 as nodata value, you will end up with edge artifacts, along with many dark pixels within the mosaic (building shadows/water etc.) being masked. Instead we use the `nearblack` program to set edge pixels with value 0-5 being considered nodata.
