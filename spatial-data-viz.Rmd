...
There are hundreds of different map projection and CRS - each with different properties and uses. The most important thing to remember is that every projection distorts the map in some way. This [mashup of map projections distortions](https://bl.ocks.org/syntagmatic/ba569633d51ebec6ec6e) provide a useful visual reference to population projection.
...
* **Global Maps**: If you are producing global maps, the preferred and modern choice is the [Equal Earth](http://equal-earth.com/) projection. Added advantage of this projection is that it preserves areas, so it is also a good choice for global scale analysis that require equal-area grids.

![The [Equal Earth](http://equal-earth.com/) Projection](images/spatial_data_viz/equal-earth.jpg)
...
![India Political Map by [Survey of India](http://www.surveyofindia.gov.in/pages/display/235-political-map-of-india) in LCC Projection](images/spatial_data_viz/india_map.jpg)
...
![UTM Zones of the world.](images/spatial_data_viz/utm_zones.png)
...
![](images/spatial_data_viz/plugins.png)
...
The exercises in this class use a variety of datasets. All the required layers, project files etc. are supplied to you in the file ``spatial_data_viz.zip`` with your purchase. Unzip this file to the `Downloads` directory.
...
3. Browse to the ``openaq.csv`` file and open it. As we want to import this file as points, select *Point coordinates*. Choose ``longitude`` as *X Field* and ``latitude`` as *Y Field*. Click *Add*.
...
![](images/spatial_data_viz/aq8_1.png)
...
![](images/spatial_data_viz/aq9_1.png)
...
27. Next, add a *Label* with the title ``Average PM2.5 Concentration (µg/m3)`` and date ``15 February, 2020``.

![](images/spatial_data_viz/aq27.png)

28. Add another *Label* with the attribution text ``Data Source: Central Pollution Control Board, EPA AirNow DOS. Downloaded from OpenAQ.org``. Now we will add a legend, so our users know how to interpret various colors on the map. Go to **Add Item &rarr; Add Legend**

![](images/spatial_data_viz/aq28.png)

29. Drag a box to insert the legend. Go to the *Item Properties* tab and turn-off *Auto update*. Remove the ``OpenStreetMap monochrome`` layer and set the ``openaq`` group label to *Hidden*.
...
![](images/spatial_data_viz/gps1.png)
...
![](images/spatial_data_viz/gps2.png)

3. Turn off the visibility of the ``sample_gps_track poinys`` layer by un-checking the box next to it. Select the ``sample_gps_track tracks`` layer and click *Open the Layer Styling Panel*. You can change the line *Color* to ``Blue`` and *Width* to ``0.5``.

![](images/spatial_data_viz/gps3.png)
...
![](images/spatial_data_viz/gps6.png)
...
![](images/spatial_data_viz/gps7.png)
...
![](images/spatial_data_viz/gps8.png)

9. In the *Effects Properties* panel, check *Outer Glow*. Select ``2.0`` for both *Spread* and *Blue radius*. 

![](images/spatial_data_viz/gps9.png)

10. Now we are ready to animate the points. Go to **Plugins &rarr; Time Manager &rarr; Toggle visibility**.

> For a more detailed guide to working with the Time Manager plugin, see [this tutorial](http://www.qgistutorials.com/en/docs/3/animating_time_series.html)

![](images/spatial_data_viz/gps10.png)

11. In the *Time Manager* panel, click the *Power* button to activate the plugin. Click *Settings*.

![](images/spatial_data_viz/gps11.png)

12. In the *Time Manager Settings* window, check the *Lopping animation* button. Next, click the *Add layer* button.

![](images/spatial_data_viz/gps12.png)
...
![](images/spatial_data_viz/gps13.png)
...
![](images/spatial_data_viz/gps14.png)

15. Once both the layers are configured, click *OK*.

![](images/spatial_data_viz/gps15.png)
...
![](images/spatial_data_viz/gps16.png)

17. Once you have the individual frames, you can use a program such as [ezgif.com](https://ezgif.com/) to create an animated GIF from them. The result would look as below.

![[View Animated GIF &#8599;](images/spatial_data_viz/gps17.gif){target="_blank"}](images/spatial_data_viz/gps17.png)
...
[Plasio](https://github.com/verma/plasio) is a browser-based LAS/LAZ point cloud viewer. Visit [plas.io](https://plas.io/). Click *Browse* and locate the ``SP5008_P_10967_20161130_20161130.laz`` file. Click *Open*. Note that this small region is rendered with over 3.5M source points.
...
![](images/spatial_data_viz/elevation1.png)

2. Switch to the *CRS* tab and search for ``EPSG:27700`` and select it. Click *OK*.

![](images/spatial_data_viz/elevation2.png)
...
![](images/spatial_data_viz/elevation3.png)

4. Click *Open the Layer Styling Panel*. Select ``Singleband pseudocolor`` renderer. Expand the *Min / Max Value Settings* section and select *Cumulate count cut*. Select a color ramp of your choice. Once styles you will be able to see the building outlines, trees, riverbed etc clearly.

![](images/spatial_data_viz/elevation4.png)
...
![](images/spatial_data_viz/elevation5.png)

6. Next, select the ``dtm_F0195499_20161130_20161130_mm_units`` layer, right-click and go to **Styles &rarr; Paste Style**.

![](images/spatial_data_viz/elevation6.png)

7. Toggle the visibility of the top layer and compare both the DSM and the DTM. You will see the DTM contains only the underlying terrain with all natural and man-made features removed.

![[View Animated GIF &#8599;](images/spatial_data_viz/elevation7.gif){target="_blank"}](images/spatial_data_viz/elevation7.png)
...
When planning urban infrastructure, a useful criteria is accessibility to public transport. This excellent article by [LiveMint](https://www.livemint.com/news/india/india-s-public-transport-challenge-1568355574941.html) analyzes the metro rail accessibility for cities across India. We will try to do a similar analysis.
...
![](images/spatial_data_viz/pop1.png)

2. Now we will query OpenStreetMap to get a vector layer of railway stations in the city. Go to **Vector &rarr; QuickOSM &rarr; QuickOSM**. OpenStreetMap uses a tagging system to record properties of physical features. The tags and the key/value pairs are described in [OpenStreetMap wiki](https://wiki.openstreetmap.org/wiki/Map_Features). To railway stations are tagged with ``railway=station``. Enter ``railway`` as the *Key*, ``station`` as the *Value* and *Bangalore* as the geographic filter *In*. Expand the *Advanced* section and check only the *Node* and *Points* boxes. Click *Run Query*.

![](images/spatial_data_viz/pop2.png)

3. Once the query finishes, you will see a new layer ``railway_station_Bangalore`` loaded in the canvas. This includes all railway stations - including the ones operator by Indian Railways and stations under constructions. We can apply a filter to select only the operation metro stations. Right-click the layer and select *Filter*.

![](images/spatial_data_viz/pop3.png)
...
![](images/spatial_data_viz/pop4.png)
...
![](images/spatial_data_viz/pop5.png)
...
![](images/spatial_data_viz/pop6.png)
...
![](images/spatial_data_viz/pop7.png)
...
![](images/spatial_data_viz/pop8.png)
...
![](images/spatial_data_viz/pop9.png)
...
![](images/spatial_data_viz/pop10.png)
...
![](images/spatial_data_viz/pop13.png)

14. We can determine the total population by summing up the values from the pixels that fall within a polygon. This operation is known as *Zonal Statistics*. Search for and locate the **Raster analysis &rarr; Zonal statistics** algorithm. Double-click to launch it.

![](images/spatial_data_viz/pop14.png)

15. Select ``ind_ppp_2000_KA`` as the *Raster layer* and ``bangalore`` as the *Vector layer containing zones*. Enter ``population_`` as the *Output column prefix*. We are interested only in the ``Sum`` so uncheck other *Statistics to calculate*. Click *Run*.

![](images/spatial_data_viz/pop15.png)

16. Once the algorithm finishes, repeat the same process for the ``metro_station_buffer_reprojected`` layer.

![](images/spatial_data_viz/pop16.png)

17. This algorithm would add a new attribute to each of these layers with the total population contained within the polygons. Use the *Identify* tool and click on the polygons. The **population_sum** field will contain the population. You can right-click on it and select *Copy Attribute Value*

![](images/spatial_data_viz/pop17.png)
...
![](images/spatial_data_viz/pop18.png)
...
![](images/spatial_data_viz/park1.png)
...
![](images/spatial_data_viz/park2.png)
...
![](images/spatial_data_viz/park3.png)
...
![](images/spatial_data_viz/park4.png)
...
![](images/spatial_data_viz/park5.png)
...
![](images/spatial_data_viz/park6.png)
...
![](images/spatial_data_viz/park7.png)
...
![](images/spatial_data_viz/park8.png)

## Exercise: Determine the Best Area for a Home

Finding optimal locations is a widely used spatial analytic technique. Using a suitability models, we can combine multiple criteria and determine rank sites. Here we will take a realistic scenario of determining the best place to live in a city using multiple criteria.
...
![](images/spatial_data_viz/overlay1.png)
...
![](images/spatial_data_viz/overlay2.png)
...
![](images/spatial_data_viz/overlay3.png)
...
![](images/spatial_data_viz/overlay4.png)
...
![](images/spatial_data_viz/overlay5.png)
...
![](images/spatial_data_viz/overlay6.png)
...
![](images/spatial_data_viz/overlay7.png)
...
![](images/spatial_data_viz/overlay8.png)
...
![](images/spatial_data_viz/overlay9.png)
...
![](images/spatial_data_viz/overlay10.png)
