...
8. Switch to the *Home* tab. Ensure that you have the *python_foundation* environment selected. We will now install some programs which allow us to write and execute Python scripts. From the home screen, search for the ``JupyterLab`` application. If it is not installed already, click the *Install* button. Windows users may optionally install the ``CMD.exe prompt`` application.
...
11. Enter the following statements in the first cell and click the *Run* button. **If nothing happens - it means your installation was successful!**. If you get an *ImportError*, repeat the installation steps carefully again. If you continue getting errors, try the *Command Line Interface* procedure outlined in the next section.
