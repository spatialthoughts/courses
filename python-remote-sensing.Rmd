...
conda install -c conda-forge -y \
  geopandas xarray rioxarray \
  dask distributed \
  pystac-client odc-stac botocore \
  duckdb \
  scikit-learn \
  xarray-spatial xvec xee exactextract \
  netcdf4 openpyxl \
  earthengine-api \
  planetary-computer \
  lonboard \
  matplotlib numpy pandas pyproj \
  jupyterlab jupyter-server-proxy \
  coiled
```
...
pip install xrscipy aef-loader
```
