...
In session, you will learn to load the scanned image and understand the georeferencing tools. Then you will collect multiple ground control points and save the scanned map as a georeferenced TIF. 
...
9. Similarly, choose a minimum of six-point. The best features to look for are rail-road intersections, building corners, city squares, or any other corner and edges. After choosing the points, click on the pop-out button in Georeferencing window to detach this tool from the main QGIS. 
...
14. Locate the point with the highest residual. We use the *Move point* or *Delete point* to adjust the point position. Since we have more GCPs, let's delete the point with high residue. Before georeferencing, a minimum of six GCPs should be available.
...
15. The total mean error and the residual for each point are lowered considerably.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/introduction_to_qgis/georef_part2_15.png')
```

16. Click on the *Start Georeferencing* button to georeference the image. 
...
In this exercise we uused a rubber sheet georeferencing (Polynomial 2), now lets try a diffrent method. [This Plate Spline](https://en.wikipedia.org/wiki/Thin_plate_spline) in this technique each pixel relative position is calculated using interpolation technique. 

1. Load the GCPs collected and the ``Banglore_1924.png`` scanned map in the georeferencing tool. 

2. In Transformation setting dialog choose *Thin Plate Spline* transformation technique. 
