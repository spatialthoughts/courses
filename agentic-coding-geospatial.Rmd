...
Use this skill when you need to convert any geospatial raster data to a Cloud-Optimized GeoTIFF.
...
3. Check if the input is already a valid COG. If it is, stop and report that. Do not re-convert.
...
* `rio-cogeo` Python package for validation. Install using `pip install rio-cogeo`

Both must come from a conda environment, not base. Check what is available before installing:

    conda env list
    conda activate <env>
    gdal --version

If neither is installed, ask the user which environment to install into before proceeding.
...
Note the flag difference. The new `gdal raster convert` takes `-f` (or `--format`).
It does NOT accept `-of`, which is the legacy `gdal_translate` flag, and fails with
"Option 'o' is not a boolean option."

Creation options are passed with `--co KEY=VALUE` on the new interface, `-co KEY=VALUE`
on the legacy one.

If the output file already exists, the command errors out. Add `--overwrite` only when
you intend to replace it.

## Compression

The COG driver already applies LZW compression by default, so the output is compressed
even if you pass nothing. But the default uses no predictor, which wastes a lot of space
on continuous data. Set the compression and predictor explicitly. Use DEFLATE as the default, since it is readable by every GeoTIFF client.

    gdal raster convert -f COG --co COMPRESS=DEFLATE --co PREDICTOR=<n> <input> <output>

Choose the predictor by band data type. Inspect the raster and read the data type from the
output.

GDAL version >= 3.11

    gdal raster info <file>

GDAL version < 3.11

    gdalinfo <file>

Both print a line per band in the form `Band 1 Block=7200x1 Type=Float32, ColorInterp=Gray`.
Read the data type from `Type=`. For scripted use, `gdal raster info -f json <file>` returns the same value at `bands[].type`.

| Data type                     | PREDICTOR | Notes                          |
| ----------------------------- | --------- | ------------------------------ |
| Float32, Float64              | 3         | Floating point predictor       |
| Byte, Int16, UInt16, Int32    | 2         | Horizontal differencing        |
| Already-compressed / RGB      | 1         | Predictor does not help        |

A wrong predictor makes files bigger so verify the type rather than guessing.

Expect the output to be larger than the input when the input had no overviews.

## File Naming Convention

Append the text `_cog` to the converted filename. If the input is `data.tif`, the output
should be named `data_cog.tif`

## Remote Files

If the given file is a remote file, do not download it. Use GDAL Virtual File Systems by
prefixing the URL with `/vsicurl/`
...
A remote input gives no obvious output location. Write to the current working directory unless a location is specified by the user.
```
