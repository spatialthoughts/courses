...
conda install -c conda-forge leafmap geopandas streamlit-folium -y
```
...
2. We now use `geopandas` to read 2 vector layers from a geopackage and `pandas` to read a CSV file containing road statistics. We put the code for data fetching inside a function and cache it. GeoDataFrames objects do not work well with the legacy `@st.cache` function, so we use the newer `@st.experimental_memo` function which caches the items by value and doesn't give an error about mutativing the items stored in the cache.
