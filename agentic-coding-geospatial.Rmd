...
4. Start the MCP server from *Plugins &rarr; QGIS MCP &rarr; Run MCP*.
...
6. Next *(Windows users)*, search for Anaconda Powershell Prompt and launch it. *(Mac/Linux users)*: Launch a Terminal window. Activate the workshop environment and install the `uv` package manager which is required by the QGIS MCP server.
...
7. Run the command to add the QGIS MCP server to Claude Code configuration.
...
8. Start Claude Code. Run the following command for managing MCP servers.
...
9. Verify that `qgis` is connected. Press *Esc* to exit.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/agentic_coding_geospatial/qgismcp9.png')
```

10. Enter the following prompt to test the connection to the QGIS MCP server. If the connection is working fine, you will get a success return message.
...
11. Let's use the MCP server. Enter the following prompt.
...
12. Claude Code will use the tools provided by the MCP server to load the data layer in QGIS.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/agentic_coding_geospatial/qgismcp12.png')
```
