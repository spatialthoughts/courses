...
This course requires QGIS LTR version 3.28. Please review [QGIS-LTR Installation Guide](install-qgis-ltr.html) for step-by-step instructions.
...
To use this class, we must create an object by instantiating it. Looking at the [class documentation](https://qgis.org/pyqgis/3.28/core/QgsDistanceArea.html), the class constructor doesn't take any arguments. We can use the default constructor to create an object and assign it to the `d` variable.
...
Hint: Look at the appropriate method in the [QgsMessageBar](https://qgis.org/pyqgis/3.28/gui/Message/QgsMessageBar.html) class
...
Hint: Use [`QgsProject.instance().setCrs()`](https://qgis.org/pyqgis/3.28/core/QgsProject.html) method.
...
Hint: [`QgsRasterInterface`](https://qgis.org/pyqgis/3.28/core/QgsRasterInterface.html#module-QgsRasterInterface) class provides a `bandStatistics()` method for calculating statistics from a raster band. You can get the reference to the instance of this class for a raster layer using `layer.dataProvider()`.
...
## 9.1 Understanding Python Iterators
...
# 10. Writing Python Console Scripts

We will now see how can we write python scripts, save them and run them within QGIS. A goal of the script is to read the features of a selected vector layer, and write its attributes to a CSV file. We will also see examples of best practices - such as validating inputs, writing files safely and communicating the status to the user.

## 10.1 Using Pandas

In this section, we will see how to combine PyQGIS with other Python libraries like Pandas.

Open the Python Console and click the *Show Editor* button. Copy/paste the following code. Click the *Save* button and save the file as `save_attributes_pandas.py`. The file can be saved anywhere.
...
## 10.2 Using PyQGIS Classes

Let's take the script we wrote above and convert it to use PyQGIS classes. This is the preferred method while developing scripts that can be used as Processing algorithms or plugins. We will use the [`QgsVectorFileWriter`](https://qgis.org/pyqgis/3.28/core/QgsVectorFileWriter.html) class to create a file in any of the supported vector data formats. Once we initialize a `QgsVectorFileWriter` object, we iterate over the original layer and call the `addFeature()` method to add features to the writer.
...
## 11.1 Writing a Processing Script
...
You can review additional information and tips in the [Writing new Processing algorithms as Python scripts](https://docs.qgis.org/3.28/en/docs/user_manual/processing/scripts.html) section of the QGIS User Guide.
...
Create these 3 files and put them in a folder named `save_attributes`. Copy the `logo.png` file from `<home folder>/Downloads/pyqgis_masterclass/logo.png` to this folder. Copy the folder to the python plugins directory at `{profile folder}/python/plugins`. 
...
Our recommended environment setup takes care of all the requirements and allows you to run PyQGIS scripts easily. The next sections describe this seup and show youhow to run the script on various platforms.
...
15. Now you know how to access the `@layer_id` and `$id` values of the current feature. We can now use it to extract the current feature and create a new layer from it. Replace the *Action Text* with the following code which uses  [`QgsFeatureSource.materialize()`](https://qgis.org/pyqgis/3.28/core/QgsFeatureSource.html#qgis.core.QgsFeatureSource.materialize) method to create a new memory based vector layer with the query containing the feature id.
...
If you use the action again, the current selection will go away and a new selection will be made. We want to change this behaviro by adding to the existing selection. Update the code for the action *Select Second-degree Neighbors* so when used, it adds the second-degree neighbors to already selected features.

Hint: Supply an additional behavior option for the  [`QgsVectorLayer.selectByIds()`](https://qgis.org/pyqgis/3.28/core/QgsVectorLayer.html#qgis.core.QgsVectorLayer.selectByIds) method.
