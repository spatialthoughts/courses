...
QGIS Plugin Manager loads the plugin from the plugin directory in the current profile. When doing plugin development, you are typically working in a different directory (mostly in a git repo) and you will have to manually copy the code from your development directory to the plugin directory for testing. [Symbolic Links](Creating Symbolic Links on Windows
...
You can use the [MKLINK command](https://www.geeksforgeeks.org/creating-symbolic-links-on-windows/) to create a symbolic link from a Windows Command Prompt.
...
The [ln command](https://www.hostinger.in/tutorials/how-to-create-symbolic-links-in-linux) can be used to create symbolic links from a terminal. Note that the `ln` command is notorious for using a non-intuitive order of the folders.

```
ln -s <plugin folder path in your dev environment> <plugin folder path in profile>
```

For example the command below creates a symlink in the plugin folder on a Mac that points to a folder on the Desktop.

```
ln -s ~/Desktop/basemap_loader /Users/ujavalgandhi/Library/Application\ Support/QGIS/QGIS3/profiles/default/python/plugins/basemap_loader
```
