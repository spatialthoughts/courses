...
Follow any one of the procedures to install Anaconda and the required environments. 

1. [Graphical user interface (GUI)][Graphical user interface (Anaconda Navigator)]
2. [Command line interface (CLI)][Command line interface (Anaconda Prompt / Terminal)]

## Graphical user interface (Anaconda Navigator) 

1. [Download the Anaconda Installer](https://www.anaconda.com/products/individual) for Python 3.8 (or a higher version) for your operating system. Once downloaded, double click the installer and install it into the default suggested directory. Select an install for *Just Me* and **use default settings**. *Note: If your username has spaces, or non-English characters, it causes problems. In that case, you can install it to a path such as  `C:\anaconda`.*
...
4. Now that the environment is created let's activate the environment. Once the environment activates, the ``(base)`` will change to ``(python_foundation)``. Moving further from here, all the modules we install in this environment cannot be accessed outside of it. Enter the below command and press Enter.
...
5. Now let's install our first module, ``geopandas``. This is a module that allows you to work with vector geospatial data in Python. A channel is a repository where packages are hosted. The default channel is suitable for most purposes. But some packages we require for this class are not available in the default channel, so we install them via ``conda-forge``. Enter the below command and press Enter. 
...
7. Let's check our installation. We will not work with CLI in this course, but let's check by accessing python via CLI for this step alone. Enter the below command and press Enter. 
...
8. If the installation is successful, you won't get any output and proceed to the following line as shown here. 

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/condacli8.png')
```

9. Let's install our remaining modules, ``geopy``, ``rasterio``, ``matplotlib`` and python user interface ``jupyter lab``. Enter the below command and press Enter. 
...
10. If all the module installation is successful, you should see a screen like this.

```{r echo=FALSE, fig.align='center', out.width='75%'}
knitr::include_graphics('images/python_foundation/condacli10.png')
```

11. Now, let's launch our web interface, ``jupyter lab``. This will initiate and run a local server in your system and opens in your default browser. Once it opens up, please do not close your anaconda prompt as it will kill your local server. Enter the below command and press Enter. 
...
12. A new browser tab will open with an instance of JupterLab. Click the *Python 3*  button under *Notebook*.
