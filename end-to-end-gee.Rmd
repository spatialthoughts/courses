...
```{js eval=FALSE, code=readLines('code/end_to_end_gee/01-Earth-Engine-Basics/04c_Mosaics_and_Composites_(exercise)')}
```
...
```{r echo=FALSE, fig.align='center', out.width='75%', fig.cap='Mosaic with MNDWI, SAVI and NDVI images'}
...
```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/06c_Calculating_Area_(exercise)')}
```

## 07. Calculating Area by Class

We learnt how to calculate area for a single class. But typically when you have a classified image, you want to compute area covered by each class. We must follow a similar process as before, but using a [Grouped Reducer](https://developers.google.com/earth-engine/reducers_grouping).

```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/07b_Calculating_Area_by_Class_(complete)')}
```
...
- *Multi Band Change*: Measuruing spectral distance and spectral angle between two multispectral images
...
[![View Presentation](images/end_to_end_gee/supervised_classification.png){width="400px"}](https://docs.google.com/presentation/d/1vdFTWJ61yDuVfbfhpnumQ8zuMPGwGcHpHsBTRgo_o5I/edit?usp=sharing){target="_blank"}
...
## 08. Direct Classification of Change
...
```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/08b_Classifying_Change_(complete)')}
```
...
```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/08c_Classifying_Change_(exercise)')}
```

## 09. Post-classification Comparison
...
```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/09b_Post_Classification_Comparison_(complete)')}
```

### Exercise

```{js eval=FALSE, code=readLines('code/end_to_end_gee/03-Supervised-Classification/09c_Post_Classification_Comparison_(exercise)')}
```
...
* Google Earth Engine [Coding Best Practices](https://developers.google.com/earth-engine/guides/best_practices)
* [#EarthEngine posts](https://spatialthoughts.com/category/earthengine/) on SpatialThoughts Blog
